<template>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-slate-500 mt-1">대한민국의 미래를 함께 논의해보세요</p>
            </div>
            <router-link 
              to="/" 
        ...
        <form @submit.prevent="submitPost" class="space-y-8">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <!-- 작성자 정보 (읽기 전용) -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">작성자</label>
                <input 
                  :value="currentUser.name || '익명 사용자'"
                  readonly
                  class="w-full px-4 py-3 bg-slate-100 border border-slate-300 rounded-lg text-slate-600"
                >
                <p class="text-xs text-slate-500 mt-1">사용자가 수동으로 변경할 수 없습니다.</p>
              </div>
            </div>
            ...
            <div class="mt-6">
              ...
              <div class="flex justify-between mt-1">
                <p class="text-xs text-slate-500">과한 어그로성 제목은 삼가해주세요</p>
                <span class="text-xs text-slate-500">{{ formData.title.length }}/100</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div>
              ...
              ></textarea>
              <div class="flex justify-between mt-1">
                <p class="text-xs text-slate-500">자유롭게 게시물을 작성해주세요.</p>
                <span class="text-xs text-slate-500">{{ formData.content.length }}자</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- 이미지 업로드 -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">이미지</label>
                ...
                <div class="mb-4">
                  <div class="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-slate-400 transition-colors">
                    ...
                    <div v-if="!imagePreview" @click="$refs.fileInput.click()" class="cursor-pointer">
                      ...
                      <p class="mt-2 text-sm text-slate-600">클릭하여 이미지 업로드</p>
                      <p class="text-xs text-slate-500">PNG, JPG, GIF (최대 5MB)</p>
                    </div>
                    <div v-else class="relative">
                      <img :src="imagePreview" alt="미리보기" class="max-h-32 mx-auto rounded">
                      <button 
                        @click="removeImage"
                        type="button"
                ...
                <!-- 또는 URL 입력 -->
                <div>
                  <label class="block text-xs text-slate-500 mb-1">또는 이미지 URL 입력</label>
                  <input 
              ...
              <!-- 유튜브 링크 -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">유튜브 동영상</label>
                <input 
                  ...
                  class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-slate-500 transition-colors"
                >
                <p class="text-xs text-slate-500 mt-1">유튜브 동영상 URL을 입력하세요</p>
  ...
  id: null,
  name: '알 수 없는 사용자' // 초기값은 게스트 또는 null
});

  onMounted(() => {
    // 컴포넌트가 마운트될 때 세션 스토리지에서 사용자 정보 로드
    ...
    if (storedUser) {
      try {
        ...
        // 파싱 실패 시 초기화하거나 에러 처리
        currentUser.value = { id: null, name: '게스트' };
